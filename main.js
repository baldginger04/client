...
async function enterApp(user) {
  ...
  // Load the user's profile + client list. A transient failure here (cold-start
  // timeout, brief network drop) must NOT log the user out — their session is
  // still valid. Retry a few times; on total failure keep them signed in and
  // offer a reload instead of calling signOut().
  let loaded = false;
  for (let attempt = 1; attempt <= 3; attempt++) {
    try {
      ...
      console.error(`loadUserContext attempt ${attempt} failed:`, err);
      if (attempt < 3) await new Promise((r) => setTimeout(r, 600 * attempt));
    }
  }
  if (!loaded) {
    // Stay signed in; let them retry without losing the session.
    if (confirm('Could not load your account data — this is usually a brief connection hiccup. Reload and try again?')) {
